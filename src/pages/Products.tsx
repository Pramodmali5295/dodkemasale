        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
          {products.map((p, i) => (
            <GsapReveal key={p.name} direction="up" delay={i * 0.1}>
              <div className="group h-full bg-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-spice/20 transition-all duration-500 border border-spice-gold/10">
                ...
                <div className="p-3 xs:p-4 sm:p-6">
                  <div className="flex items-center gap-1.5 mb-1.5 sm:mb-2">
                    <span className="text-[8px] xs:text-[10px] font-bold text-spice-red uppercase tracking-widest bg-spice-red/5 px-2 py-0.5 rounded-md">{p.category}</span>
                  </div>
                  <h3 className="font-display text-sm xs:text-base sm:text-xl font-bold mb-1 sm:mb-2 text-spice-dark group-hover:text-spice-red transition-colors line-clamp-1">{p.name}</h3>
                  <p className="font-body text-muted-foreground text-[10px] sm:text-xs mb-3 sm:mb-4 leading-normal sm:leading-relaxed line-clamp-2 h-7 sm:h-10">{p.desc}</p>
                  
                  <div className="flex items-center justify-between border-t border-spice-gold/5 pt-3 sm:pt-4 mt-auto">
                    <div className="hidden xs:flex items-center gap-2 text-spice-green font-bold text-[8px] sm:text-[10px] uppercase tracking-wider">
                      <span>Verified quality</span>
                    </div>
                    <Link
                      to="/contact"
                      className="inline-flex items-center gap-1.5 sm:gap-2 text-spice-dark font-display font-bold text-[10px] sm:text-xs group-hover:text-spice-red transition-all ml-auto xs:ml-0"
                    >
                      Enquire
                    </Link>
                  </div>
